...
public enum AppPaths {
    /// ~/Library/Application Support/Skillbox — settings, lockfile, logs, registry.
    public static func appSupportDirectory() -> URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        return base.appendingPathComponent("Skillbox")
    }
}
...
public final class SettingsStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "skillbox.settings")
...
public final class LogStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "skillbox.logs")
    private let fileURL: URL
